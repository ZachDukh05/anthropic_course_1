...
test("renders chat interface with message list and input", () => {
  // Provide messages so the component renders MessageList instead of the empty state
...
test("scrolls when messages change", () => {
  // Start with messages so the scroll area and MessageList are rendered
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [{ id: "1", role: "user", content: "Hello" }],
  });
...
test("renders with correct layout classes", () => {
  // Provide messages so MessageList and the ScrollArea wrapper are rendered
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [{ id: "1", role: "user", content: "Hello" }],
  });
